...
export function TreeRow({
  ...
  const openPath = row.kind === 'close' ? row.path.replace('__close', '') : row.path;
  ...
  return (
    <div className={cls} onClick={() => onSelect(openPath)}>
...
function PrimContent({ row }: { row: Extract<Row, { kind: 'prim' }> }) {
  return (
    <>
      ...
      {typeof row.value === 'string' && <span className="s">"{row.value}"</span>}
