...
export function App({ rawJson }: AppProps) {
  ...
  const matches = useMemo<string[]>(() => {
    ...
    return rows
      .filter((row) => {
        const text = caseSen ? rowSearchText(row) : rowSearchText(row).toLowerCase();
  ...
  const rawStr = useMemo(() => JSON.stringify(data, null, 2) ?? rawJson, [data, rawJson]);
  ...
  const handleCollapseAll = useCallback(() => {
    ...
    function walk(val: unknown, path: string) {
      if (val !== null && typeof val === 'object') {
        ...
        keys.forEach((k) =>
          walk(
            isArr ? (val as unknown[])[Number(k)] : (val as Record<string, unknown>)[k],
            isArr ? `${path}[${k}]` : `${path}.${k}`,
          ),
        );
      }
